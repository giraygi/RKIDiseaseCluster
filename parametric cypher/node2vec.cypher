...
RETURN {
  hashgnnparams: $hashgnnparams,
  node2vecparams: $node2vecparams,
  graphsageparams: $graphsageparams,
...
CALL gds.graph.exists('transmits') YIELD exists AS exists1
WITH exists1
WHERE exists1 = true
...
CALL gds.graph.exists('transmits13') YIELD exists AS exists2
WITH exists2
WHERE exists2 = true
CALL gds.graph.drop('transmits13') YIELD graphName
RETURN 'Graph dropped: ' + graphName AS message;

CALL gds.beta.pipeline.exists('linkPredictionPipeline') YIELD exists AS exists3
WITH exists3
WHERE exists3 = true
...
CALL gds.model.exists('linkPredictionModel') YIELD exists AS exists4
WITH exists4
WHERE exists4 = true
...
{hiddenLayerSizes: [$generalparams.hiddenLayerSize1, $generalparams.hiddenLayerSize2], penalty: $generalparams.penalty, patience: $generalparams.patience}) YIELD parameterSpace
